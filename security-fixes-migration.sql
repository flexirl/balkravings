...
-- ============================================================
-- FIX 1: Server-side secure order creation
-- Calculates total from actual DB prices, not client-provided values
-- ============================================================
...
CREATE OR REPLACE FUNCTION public.create_secure_order(
  ...
  -- p_items format: [{"food_id": "uuid", "quantity": 2}, ...]
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
      ...
      IF v_coupon.expires_at IS NULL OR v_coupon.expires_at > now() THEN
        ...
        IF v_coupon.usage_limit = 0 OR v_coupon.used_count < v_coupon.usage_limit THEN
          ...
          IF v_subtotal >= v_coupon.min_order THEN
            -- Calculate discount (skip for freebie type)
...
-- ============================================================
-- FIX 2: Force payment_status = 'pending' on all new orders
-- Prevents users from inserting orders with payment_status='paid'
-- ============================================================

CREATE OR REPLACE FUNCTION public.enforce_pending_payment()
RETURNS TRIGGER AS $$
BEGIN
  -- Always force these values on INSERT (can't be set by client)
...
DROP TRIGGER IF EXISTS enforce_pending_on_insert ON public.orders;
CREATE TRIGGER enforce_pending_on_insert
  BEFORE INSERT ON public.orders
  FOR EACH ROW
  EXECUTE FUNCTION public.enforce_pending_payment();


-- ============================================================
-- FIX 3: Prevent admin role escalation
-- Users should ONLY be able to update their name and phone
-- Cannot change: role, is_blocked, block_reason
-- ============================================================

-- Drop the old permissive policy
DROP POLICY IF EXISTS "Users can update own profile" ON public.profiles;

-- Create a restricted policy: users can only update name and phone
-- We use a USING clause to match their own row, and a 
-- WITH CHECK that ensures role and is_blocked haven't changed
CREATE POLICY "Users can update own profile (restricted)"
  ON public.profiles FOR UPDATE
  USING (auth.uid() = id)
  WITH CHECK (
    -- Ensure role hasn't been changed from what's in the DB
    role = (SELECT role FROM public.profiles WHERE id = auth.uid())
    AND
    -- Ensure is_blocked hasn't been changed
